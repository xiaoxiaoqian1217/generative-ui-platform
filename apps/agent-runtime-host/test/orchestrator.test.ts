...
describe("RunOrchestrator", () => {
  it("maps Business Agent content through the embedded pipeline", async () => {
    ...
    expect(result).toMatchObject({
      ...
      threadId: "thread-1",
      runId: "run-1",
      presentation: { mode: "markdown", markdown: "# Safe result\n" },
    });
    ...
    const orchestrator = createRunOrchestrator({
      businessAgentAdapter: new MockBusinessAgentAdapter({
        run: async (request) => ({
          ...
          runId: request.runId,
          status: "completed",
          content: { contentType: "markdown", markdown: businessContent },
        }),
        resumeAction: async () => {
          throw new Error("not used");
        },
  ...
  it("rejects actions for an unknown Surface without resuming the Business Agent", async () => {
    const result = await createOrchestrator().action({
      protocolVersion: "1.0",
  ...
  it("validates, atomically consumes, resumes, and re-presents an approved Catalog Action", async () => {
    ...
    if (
      run.status === "failed" ||
      run.presentation.status !== "completed" ||
      run.presentation.mode !== "generative-ui"
    )
    ...
    const base = {
      ...
      action: {
        ...
        surfaceId: run.presentation.surfaceId,
      },
    };
    await expect(
      orchestrator.action({ ...base, requestId: "request-no-approval" }),
    ...
    await expect(
      orchestrator.action({
        ...
        action: { ...base.action, approved: true, payload: { forged: true } },
      }),
    ).resolves.toMatchObject({
      status: "failed",
    ...
    await expect(
      orchestrator.action({
        ...
        action: { ...base.action, approved: true },
      }),
    ).resolves.toMatchObject({
      status: "completed",
      sourcePresentationRequestId: run.presentationRequestId,
      presentation: { mode: "generative-ui" },
    });
  ...
  it("routes a Business Agent text-confirmation intent through Runtime Action validation", async () => {
    let resumes = 0;
    const orchestrator = createRunOrchestrator({
      businessAgentAdapter: new MockBusinessAgentAdapter({
        run: async (request) => ({
          ...
          status: "completed",
          content:
            request.input.message === "confirm"
              ? {
                  contentType: "structured-data",
                  data: {
                    kind: "confirmation-intent",
                    pausedRunId: "run-draft",
                    actionId: "confirm-patrol-plan",
                    actionType: "patrol.confirm",
                  },
                  fallbackMarkdown: "Confirmation intent",
                }
              : {
                  contentType: "structured-data",
                  data: { kind: "patrol-plan-draft", summary: {} },
    ...
    const confirmed = await orchestrator.run({
      protocolVersion: "1.0",
