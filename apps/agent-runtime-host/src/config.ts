...
export type RuntimeHostConfigErrorCode = "INVALID_PORT";

export class RuntimeHostConfigError extends Error {
  readonly code: RuntimeHostConfigErrorCode;

  constructor(code: RuntimeHostConfigErrorCode, message: string) {
    super(message);
    this.name = "RuntimeHostConfigError";
    this.code = code;
  }
}
...
function parsePort(value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new RuntimeHostConfigError(
      "INVALID_PORT",
      `PORT 必须是 1 到 65535 之间的整数，当前值为：${value}`,
    );
  }
