...
const catalog: ComponentCatalog = {
  ...
  components: [
    { type: "Markdown", description: "Sanitized Markdown", propsSchema: {} },
  ],
};
...
createServer(async (request, response) => {
  ...
  if (
    request.method === "POST" &&
    ["/api/ui-compiler/compile", "/ag-ui"].includes(request.url ?? "")
  ) {
    try {
      const body = await readJson(request);
      const parsed = uiCompileRequestSchema.safeParse(body);
      if (!parsed.success)
        return sendJson(response, 400, { error: parsed.error.flatten() });
      const result = await compileUI(parsed.data as UICompileRequest, {
        catalog,
      });
      if (request.url === "/ag-ui") {
        return sendJson(
          response,
          200,
          compileResultToAgUiEvents(
            parsed.data.runId ?? parsed.data.requestId,
            result,
          ),
        );
      }
...
}).listen(port, () => console.log(`ui-compiler-service listening on ${port}`));
