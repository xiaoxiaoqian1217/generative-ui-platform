...
/**
 * The conversation keeps only a slim pending record and the final decision
 * record; the decision UI lives on the map overlay. This component registers the
...
const hintText = computed(() => {
  if (consultRevisionMode.value && props.variant === "c")
    return "修改模式: 已在选定路线上添加锚点, 请描述修改要求。";
...
const doneText = computed(() => {
  ...
  const revisedOption = request.value.success
    ? request.value.data.options.find(
        (candidate) => candidate.id === result.selectedOptionId,
      )
    : undefined;
  ...
  return `已记录 ${revisedOption?.label ?? result.selectedOptionId} 修改要求: ${result.instruction}。${anchorNote}`;
});
</script>
