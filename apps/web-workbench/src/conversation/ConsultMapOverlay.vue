...
/**
 * PROTOTYPE (variants B and C): map-overlay decision UI for the patrol-route
 * consultation. Reads the active consultation registered by the slim chat
 * record and renders:
...
async function confirmTentative(): Promise<void> {
  ...
  if (session.value === undefined || option === undefined) return;
...
function switchPopupToRevision(): void {
  const anchor = consultPopupAnchor.value;
  const option = popupOption.value;
  if (anchor === undefined || option === undefined) return;
...
watch(showRevisionPopup, (visible) => {
  if (visible) {
...
function closeRevisionPopup(): void {
  consultRevisionPopup.value = undefined;
  consultRevisionAnchor.value = undefined;
  consultRevisionMode.value = false;
}
...
<template>
  <div class="consult-map-overlay" aria-hidden="false">
    ...
    >
        ...
        <span>
          {{
            tentativeOption === undefined
              ? "第 1 步: 先选择路线 A 或路线 B"
              : `第 2 步: 点击地图中的${tentativeOption.label}可提出修改`
          }}
        </span>
      </div>
      ...
      <div class="consult-dock-footer">
        ...
        <button
          ...
          :disabled="tentativeOption === undefined"
          type="button"
    ...
    >
      ...
      <p class="consult-revision-anchor" data-testid="revision-anchor-label">
