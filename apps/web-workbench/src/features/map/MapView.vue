...
function consultCandidateKey(
  ...
  return candidates
    .map(
      (candidate) =>
        `${candidate.featureId}:${candidate.color}:${candidate.letter}`,
    )
    .join("|");
}
...
function applyConsultCandidateHandlers(clickable: boolean): void {
...
onMounted(() => {
  ...
  applyConsultCandidateHandlers(props.consultCandidatesClickable);
...
watch(
  ...
  (clickable) => applyConsultCandidateHandlers(clickable),
);
