...
export interface ConsultRouteCandidateHandlers {
  readonly isClickable: (featureId: string) => boolean;
...
export function createMapController(
  ...
  function isConsultCandidateClickable(featureId: string): boolean {
    return consultCandidateHandlers?.isClickable(featureId) === true;
  }

  function syncConsultBadgeInteractivity(): void {
    for (const marker of consultCandidateMarkers) {
      const badge = marker.getElement();
      if (!(badge instanceof HTMLButtonElement)) continue;
      badge.disabled = !isConsultCandidateClickable(
        badge.dataset.featureId ?? "",
      );
    }
  }
  ...
  function bindConsultCandidateEvents(): void {
    ...
    map.on("mousemove", CONSULT_CANDIDATES_HIT_LAYER_ID, (event) => {
      ...
      map.getCanvas().style.cursor =
        featureId === undefined || !isConsultCandidateClickable(featureId)
          ? ""
          : "pointer";
    ...
    map.on("click", CONSULT_CANDIDATES_HIT_LAYER_ID, (event) => {
      const featureId = event.features?.[0]?.properties?.featureId as
        | string
        | undefined;
      if (featureId === undefined || !isConsultCandidateClickable(featureId))
        return;
  ...
  function createConsultBadgeElement(
    ...
    badge.dataset.featureId = candidate.featureId;
    badge.dataset.testid = `consult-route-badge-${candidate.featureId}`;
    badge.disabled = !isConsultCandidateClickable(candidate.featureId);
    badge.setAttribute("aria-label", candidate.label);
    badge.style.background = candidate.color;
    badge.textContent = candidate.letter;
    badge.addEventListener("mouseenter", () => {
      if (isConsultCandidateClickable(candidate.featureId))
        consultCandidateHandlers?.onHover(candidate.featureId);
    });
    ...
    badge.addEventListener("click", (event) => {
      ...
      if (!isConsultCandidateClickable(candidate.featureId)) return;
  ...
  /**
   * Showing candidates frames the decision: fit the viewport to the candidate
    ...
    async showConsultRouteCandidates(candidates) {
      ...
      for (const candidate of candidates) {
        ...
        const marker = new maplibregl.Marker({
          element: createConsultBadgeElement(candidate, position),
        })
          .setLngLat(position)
          .addTo(map);
