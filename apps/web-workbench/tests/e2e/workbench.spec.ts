...
async function clickConsultRoute(
  page: Page,
  featureId: "patrol-path-a" | "patrol-path-b",
) {
  const badge = page.getByTestId(`consult-route-badge-${featureId}`);
  await expect(badge).toBeVisible();
  const box = await badge.boundingBox();
  expect(box).not.toBeNull();
  if (box === null) throw new Error(`Route badge ${featureId} has no bounds.`);
...
test("accepted decision dock requires a route choice before map-anchored revision", async ({
  ...
  await clickConsultRoute(page, "patrol-path-b");
  ...
  await expect(dock).toContainText("第 2 步: 点击地图中的路线 B可提出修改");
  await expect(confirm).toBeEnabled();
  await expect(confirm).toHaveText("确认选择路线 B");

  await clickConsultRoute(page, "patrol-path-a");
  await expect(page.getByTestId("consult-revision-popup")).toHaveCount(0);
