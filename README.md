...
当前仓库已具备最小 Workspace、共享契约包和 UI Compiler Core 的 summary 场景确定性编译链路。
活动代码树包含 UI Compiler Service、六个共享包和 UI Compiler Core。
...
UI Compiler Service 已实现 ADR-0014 Policy 1.0 Markdown Sanitizer、结构化数据资源校验和确定性 Markdown 直出 tracer bullet。
结构化数据直出支持清理调用方提供的 Fallback，并在没有可用 Fallback 时完整、稳定地序列化 JSON。
当前实现不包含 generative UI 模型路径、Core 编排、HTTP、前端或真实业务 Agent 集成。
