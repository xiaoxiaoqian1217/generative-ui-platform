...
AG-UI 解决的是前端与 Agent Runtime Host 之间的 Run、Step、消息和事件传输问题。
...
当 Copilot Runtime 或其他 Agent Runtime Host 已经拥有 Business Agent Run 时，该设计会产生两层 Run 生命周期、两套关联标识、重复的取消和错误映射，以及不清晰的执行语义。
...
本 ADR 不授权创建或实现 Agent Runtime Host、Interaction Gateway 或真实业务 Agent。
...
UI Compiler Core 不输出 AG-UI 事件，不管理 AG-UI Run 生命周期，也不依赖 CopilotKit、AG-UI SDK 或任何 Agent Runtime Host。
UI Compiler Service 不负责 Business Agent Run 的 `RUN_STARTED`、`RUN_FINISHED`、`RUN_ERROR`、Step、Thread 或事件流编排。

Agent Runtime Host 或调用方 Adapter 负责：
...
- 维护属于 Business Agent Run 的关联标识、取消、错误和终止语义。
...
- Agent Runtime Host 可以选择 AG-UI、WebSocket、SSE、gRPC 或其他传输协议。
- 业务 Agent 只需要提供协议无关的业务调用接口，不需要实现 AG-UI。
- `threadId` 和 `runId` 继续作为可选关联字段存在于 Presentation 契约中，但 Compiler 不拥有 Business Agent Run 生命周期。
- `packages/ag-ui-adapter` 如果保留，只能作为可选协议工具包，不属于 Compiler Core，也不构成 UI Compiler Service 的必需接口。
- 新增 Copilot Runtime、其他 Agent Runtime Host 或 Interaction Gateway 仍然需要显式范围变更 Issue 和新的 ADR。
