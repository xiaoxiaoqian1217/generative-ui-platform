...
- UI Compiler 不存在脱离 Agent Runtime Host 的已确认独立远程调用方。
- 当前业务规模、团队边界、发布周期和运行规模不需要为 UI Compiler 建立独立服务 SLA 或独立扩容单元。

在该前提下继续保留独立 UI Compiler Service，会为一次 Agent Run 增加内部 HTTP 调用、独立端口、服务发现、健康检查、部署配置、跨服务超时、跨服务追踪和额外故障边界。
这些成本不能直接提高当前平台的核心验证价值。
...
- 业务前端应只接入一个平台后端入口。
- Business Agent 输出和展示编译属于同一次 Agent Run 的处理生命周期。
- Agent Run 的 requestId、runId、取消信号、超时预算、降级和可观测上下文应在同一进程内连续传递。
...
Agent Runtime Host 成为平台统一后端运行时和应用组合根。
它在完成 Business Agent Run 或收到可展示的 AgentContent 后，直接调用进程内的 Presentation Pipeline。
...
当前阶段不保留独立 UI Compiler HTTP Service、UI Compiler Client 或 `UI_COMPILER_MODE=remote` 双模式。
未来是否重新建立远程服务，必须由新的需求证据和 ADR 决定。

### 代码和模块边界

原 `apps/ui-compiler-service` 中与 HTTP、CLI 和独立进程生命周期无关的展示应用能力，必须迁移为独立 Package。
推荐使用：
...
- 接收 `PresentationRequest` 或等价的内部展示请求。
- 清理 Markdown。
- 校验和安全序列化结构化数据。
- 加载并校验 Component Catalog。
- 调用 Presentation Router。
- 在需要语义分析时调用 Model Adapter。
- 校验模型产生的候选展示决策和 UI Plan Candidate。
- 调用 UI Compiler Core。
- 生成 `PresentationResult`。
- 在模型、路由或编译失败时执行安全 Markdown 降级。
- 提供供应商无关的可观测性 Port。

Agent Runtime Host 负责组装该 Package 的具体依赖，但不得把展示规则、Catalog 规则、模型供应商响应类型或 Core 编译逻辑复制到 Runtime 层。
...
合并部署后，它运行在 Agent Runtime Host 进程内，但只能由 Presentation Router 或 Presentation Pipeline 调用。
...
+ Catalog 能力摘要
        |
        v
展示决策或 UI Plan Candidate
```

Model Adapter 不负责 Business Agent 意图识别、业务工具调用、业务状态、任务规划或流程恢复。
Business Agent 如需大模型，必须使用独立的业务模型客户端和业务契约，不得复用 Presentation Model Adapter 的提示词和职责。
...
Presentation Pipeline 属于 Agent Run 的展示后处理阶段。
它共享当前 Run 的关联标识、取消信号和总超时预算。
...
安全 Markdown 降级
        |
        v
Agent Run 返回可消费结果和安全诊断
```

只有输入本身无法安全消费、Catalog 无法建立可信边界或 Runtime Host 无法生成任何安全结果时，才允许返回失败结果。

### 可观测性

ADR-0017 的敏感数据限制、稳定字段、阶段耗时和供应商无关 Observability Port 继续有效。

实现迁移后，HTTP 请求级生命周期由 Agent Runtime Host 统一拥有。
Presentation Pipeline 只记录展示阶段事件，不再拥有独立 UI Compiler HTTP 请求终局。

事件名称可以迁移为 Runtime Host 下的稳定命名空间，但不得放宽以下限制：
...
当前未采用，因为没有已确认的独立调用方、独立 SLA、独立团队或独立扩容需求。
...
- Generative UI Compiler 在当前平台阶段保持独立部署。
- Runtime Host 通过 UI Compiler Client 调用 UI Compiler Service。
- Model Adapter 的进程归属固定为独立 UI Compiler Service。

ADR-0018 关于仓库平台范围、Business Agent 边界、Web 统一入口、Frontend Runtime、Action 闭环和 Interaction Gateway 非目标的其他结论继续有效。

本 ADR 部分取代 ADR-0016 中以独立 UI Compiler HTTP Service 为前提的 HTTP 生命周期归属。
ADR-0016 的客户端断开、取消、超时、优雅关闭和稳定错误语义应迁移到 Agent Runtime Host 的统一 HTTP 生命周期中。
...
- 业务接入通常只需部署一个平台后端 Runtime Host。
- 完整开发验证环境减少一个独立后端进程、端口和健康检查。
- Agent Run、Business Agent 输出、展示编译和降级共享同一取消、超时和观测上下文。
...
- Agent Runtime Host 进程承担更多 CPU、内存和模型调用压力。
- 模型供应商故障可能在同一进程内占用 Runtime 资源。
- Runtime Host 与 Presentation Pipeline 需要严格的超时、并发和异常隔离。
- 当前不支持 Compiler 独立扩容和独立发布。
- 迁移期间需要同时调整代码、文档、边界检查、测试和启动命令。
- 如果未来出现跨语言或企业共享 Compiler 场景，需要重新建立远程服务宿主。

## 重新评估触发条件

出现以下任一明确需求时，应重新评估独立 UI Compiler Service：

- 非 Agent Runtime Host 系统需要通过网络直接调用 Compiler。
- 两个或以上逻辑独立的 Runtime Host 需要共享统一 Compiler。
- Python、Java、Go 或其他非 Node Runtime 需要使用同一 Compiler 实现。
- Compiler 模型调用需要独立资源池、限流、扩容或 SLA。
- Runtime Host 与 Compiler 由不同团队维护或采用不同发布周期。
- Compiler 需要成为企业公共远程服务或对外 API。
...
1. 从 `apps/ui-compiler-service` 提取 Presentation Pipeline 应用能力到独立 Package。
2. 保持现有契约、Sanitizer、Router、Model Adapter、Catalog 和 Core 测试通过。
3. 在 Agent Runtime Host 中组装 Presentation Pipeline。
4. 将 AgentContent 到 PresentationResult 的处理接入 Run Orchestrator。
5. 将 Action 恢复后的新 AgentContent 重新接入同一 Pipeline。
6. 把 HTTP 生命周期、取消、总超时和请求级可观测性统一迁移到 Agent Runtime Host。
7. 增加 Runtime Host 内部 Presentation Pipeline 集成测试。
8. 增加 Fixture Model 下的完整浏览器 E2E。
9. 更新 README、Requirements、Architecture、Context、Goal 和 AGENTS 规则。
10. 验证不存在 `apps -> apps` 依赖和已废弃 UI Compiler Client。
11. 完成全部迁移和回归后再删除独立 `apps/ui-compiler-service` 应用入口。

迁移期间不得为了提前删除 Service 而复制业务逻辑、放宽 Schema、绕过 Catalog 或降低安全降级能力。

## 验证要求

该决策通过以下证据持续验证：

- 默认平台启动不再要求独立 UI Compiler Service 进程或端口。
- Agent Runtime Host 是 Web 的唯一后端入口。
- Runtime Host 通过独立 Presentation Pipeline 接口处理 AgentContent。
- `ui-compiler-core` 不依赖 Runtime Host、Business Agent、模型 SDK或网络框架。
- Model Adapter 只被 Presentation Pipeline 调用。
...
- Fixture 模式下完整平台 E2E 可以在 CI 中稳定重复执行。
