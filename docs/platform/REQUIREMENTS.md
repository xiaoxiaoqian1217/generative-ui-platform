...
仓库当前已不再只验证 UI Compiler Core 和 UI Compiler Service。
Agent Runtime Host、Web Demo 和 Generative UI Workbench 已进入仓库，下一阶段需要验证 Business Agent 接入、UI Compiler 调用、浏览器 A2UI 渲染和 Action 回传的完整链路。
...
- Runtime Host 调用 UI Compiler Service；
- UI Compiler Service 通过 Model Adapter 产生展示决策或 UI Plan Candidate；
...
- UI Compiler Client；
- UI Compiler Model Adapter 多供应商验证；
...
Web 不得直接调用 Business Agent、UI Compiler Service 或模型供应商。
...
平台当前所称 Model Adapter 属于 UI Compiler Service。
...
- 新克隆仓库可冻结安装；
- 一个命令启动 Workbench、Runtime Host、Reference Business Agent 和 UI Compiler Service；
- Fixture 模式不需要模型密钥；
