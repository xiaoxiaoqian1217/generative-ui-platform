...
3. 复用 Issue #213 Scenario Lab 的场景文件和人工审定经验，但为地图交互单独定义 Inspect 证据导出、录屏和评分方式。
4. 按本文准入 dev-only 薄验证 Agent source，并在实施 Issue 中固定开关、source 标识、配置留证与时间盒。
5. 场景 A-D 的实现、运行、体验评估和证据导出全部在当前项目完成，不增加智能参谋项目集成步骤。
6. 与 `docs/ARCHITECTURE.md` 对齐工具边界、Runtime 职责和 SACS capability gap 后再进入实现。
7. 如果验证后决定把薄验证 Agent 留在产品默认拓扑，或把交互 taxonomy 提升为正式契约，再新增 ADR。
