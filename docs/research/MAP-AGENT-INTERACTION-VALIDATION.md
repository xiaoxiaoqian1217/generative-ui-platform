...
本文验证的核心问题：当 Agent 连续操作共享地图时，用户能否理解变化、归因意图并保持控制感。
所有验证都在一个固定业务事实的母场景中发生，不堆叠独立 demo。

### 三套编号的命名规则

本文使用三套相互正交的编号，阅读时请先区分：

| 编号 | 含义 | 详细定义 |
| --- | --- | --- |
| 母场景 | 固定业务语境与 fixture 数据包，本身不是可执行场景 | 场景设计 |
| A-D | 核心交互场景：同一任务中按用户介入方式切分的四种模式 | 场景地图、验证矩阵 |
| E-H | 横切验证维度：注入 A-D 观察的异常与状态方面，不是独立场景 | 场景地图、验证矩阵 |
| A0-A2 | 场景 A 的三种 UI 呈现变体，用于形成性对照实验 | 用户感知设计 |

### 场景关系

```text
母场景：北侧通道巡逻方案研判与调整（固定业务事实与地图投影）
  |
  +-- 核心交互场景（按用户介入程度递进）
  |     A 多步工具中介 -> A0/A1/A2 呈现变体实验
  |     B 征询等待
  |     C 打断纠偏（A/B 进行中用户语言修改目标）
  |     D 混合主导（A/B 进行中用户手动接管地图）
  |
  +-- 横切验证维度（注入 A-D 观察）
  |     E 失败路径 / F 意图可见性 / G 状态残余 / H 恢复与接续
  |
  +-- 基线对照
        单轮问答 / 委托执行 / 单步工具（locateDevice 迁移）
```

### 当前进度

场景 A 的确定性工程基线已由 Issue #214 完成，保留为 A0 对照。
A1 最小语义反馈与场景 B 征询等待的工程闭环已经实现，正式用户感知评估仍待执行。
Issue #216 已落地独立、默认关闭的 `map-validation-agent` 及三个版本化场景，但真实模型 smoke、重复运行证据与正式可靠性结论仍待补充。
场景 C、D 与横切维度 E-H 待实施，逐项状态见"验证矩阵"与"实施顺序"。
...
### 场景地图：核心场景 A-D、横切维度 E-H 与呈现原型 A0-A2

这三套编号不是同一层概念，定义如下。

**核心交互场景 A-D** 是同一个母场景任务流程中按"谁在主导、用户何时介入"切分的四种交互模式：

| 场景 | 交互模式 | 用户介入方式 |
| --- | --- | --- |
| A 多步工具中介 | Agent 连续执行多个地图域意图 | 不介入 |
| B 征询等待 | Agent 在业务选择处停下，请用户决定 | 语言答复 |
| C 打断纠偏 | 用户在执行中途修改业务目标 | 语言打断 |
| D 混合主导 | 用户直接操作共享地图 | 手动接管 |

A 到 B 是时间上的先后阶段（Agent 先展开，再征询），C 与 D 是用户在 A 或 B 进行中介入的分支。

**横切验证维度 E-H** 不是独立场景，而是注入 A-D 中去观察的方面：

| 维度 | 观察什么 |
| --- | --- |
| E 失败路径 | 错误归因、残余状态与恢复边界 |
| F 意图可见性 | 用户能否归因每次地图变化的发起者与理由 |
| G 状态残余 | 临时预览、Agent 高亮与用户固定状态的保留规则 |
| H 恢复与接续 | 新旧 Run 的衔接与被替代操作的处置 |

**呈现原型 A0-A2** 只是场景 A 的三种 UI 呈现变体，用于形成性对照实验：

| 原型 | 呈现范围 |
| --- | --- |
| A0 | 地图终态与最终 Assistant 文本（当前基线） |
| A1 | 最小语义反馈：当前动作、关键完成事实、临时预览与结果边界 |
| A2 | 持久过程视图：可更新步骤、等待、失败、取消、恢复和按需详情 |

三个变体的 Agent 输入、Tool Call 顺序、地图终态与 fixture 事实完全一致，只改变用户看到多少过程信息。

此外还有三个基线对照：单轮问答、委托执行与单步工具调用（locateDevice 迁移）。
每个场景、维度与原型的完整可证伪假设、必须通过项与现状见"验证矩阵"，A0-A2 的实验设计见"用户感知设计"。
...
核心交互场景回答"人与 Agent 如何共同完成任务"，横切验证维度回答"任一核心场景发生异常、留下状态或需要解释时，系统是否仍然可理解、可恢复"。
编号定义见"场景地图"，本节给出每项的可证伪假设、必须通过项与现状。
...
| 委托执行："帮我想想怎么巡逻北侧通道" | 作为完整地图协作任务入口 | Agent 使用给定业务事实组织地图展示与路线比较，不生成新路线 | Run lifecycle | 场景 A 与 B 工程基线已完成；完整协作待 C-D |
...
| B 征询等待：Agent 依次介绍并预览候选路线 A / B，请用户选择、取消或提出修改要求 | Agent 能在业务选择处停止，地图预览能够支持决策而不声称路线由 Agent 计算 | 答复前不提交或执行路线；候选路线可独立预览；选择、取消和修改要求都能闭环 | previewPath、useHumanInTheLoop、地图本地预览 | 确定性工程闭环已完成；真实模型行为证据待补 |
...
当前 AGUIMock 场景 A 发送 `map-plan` `ACTIVITY_SNAPSHOT`、四组真实 Tool Call、对应 Tool Result 和最终 Assistant 文本。
它已经提供公开 Plan、Action、操作 Evidence 和 Result 的基础事实，但没有发送结构化分析 Evidence 或 `STEP_*`。
map-plan 计划由 fixture 受控声明：title 与 goal 固定，三个计划步骤与"用户感知契约"给出的候选最小语义里程碑同构，第一步合并 `setLayerVisibility` 与 `focusOn`，步骤状态由已完成操作数确定性推导，快照随每个操作推进对同一 messageId 重复更新。
Workbench 只投影 Agent 发送的公开计划和真实工具结果，不补写计划、分析理由或未发生的操作。
...
fixture 中的 map-plan 内容是人工审定的受控声明，只证明"Agent 公开计划 -> `ACTIVITY_SNAPSHOT` -> Workbench 投影"链路与语义里程碑形态在工程上可行。
它不证明真实 LLM Agent 能稳定产生有用户意义的公开计划，这留作薄验证 Agent 阶段的行为证据问题。
如果真实 Agent 能稳定提供公开计划，沿用 `activityType: "map-plan"` 的 `ACTIVITY_SNAPSHOT` 契约，再评估是否通过 `ACTIVITY_DELTA` 增量更新计划状态。
在此之前不由 Runtime 或 Workbench 伪造 `STEP_*`、额外 `ACTIVITY_*` 或 Agent 计划。
...
1. 如果 A1 已满足理解与归因目标，优先使用现有 RUN_*、TOOL_CALL_*、TOOL_CALL_RESULT、受控 map-plan ACTIVITY_SNAPSHOT 和 Workbench 地图事实完成薄语义投影。
...
5. 已完成工程实现，待体验评估 - 场景 A 用户感知验证：保留 Issue #214 为 A0，A1 已提供最小语义反馈，仍需通过参与者任务决定是否需要 A2 持久过程视图。
6. 已完成工程实现，待真实模型证据 - 征询等待闭环：Vue v2 `useHumanInTheLoop` 已覆盖路线 A / B 本地预览、选择、取消和修改要求。
7. 已完成接线，待真实模型 smoke 与重复运行 - 薄验证 Agent：Issue #216 已落地独立 LangGraph server、条件注册、独立模型配置和三个版本化场景；首轮行为证据仍按评估协议收集。
...
若此方向准备从 dev-only 验证提升为默认产品能力或正式交互契约：

1. 保留 Issue #214 已完成的地图域意图工具与场景 A 工程基线，将其作为 A0 对照，不把用户理解或正式体验评估追溯为该 Issue 的完成条件。
2. 按"用户感知契约"比较已实现的 A0、A1 与必要时的 A2，形成场景 A 的意图可见性证据，再决定是否实现持久过程呈现。
3. 为地图交互单独定义 Inspect 证据导出、录屏和评分方式，不只复用 Issue #213 Scenario Lab 的场景文件与人工审定经验。
4. 保持 Issue #216 已固定的薄验证 Agent 开关、source 标识、独立配置与时间盒边界，并补齐真实模型运行证据。
