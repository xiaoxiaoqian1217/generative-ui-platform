...
当前仓库已经具备 UI Compiler Core、UI Compiler Service、Presentation Contract、Component Catalog Schema、Agent Runtime Host，以及 HTTP 和 WebSocket Mock 验证。

但现有能力仍以局部验证为主，主要链路彼此分离：
...
因此当前还不能从浏览器完整验证 Business Agent 接入、展示决策、受控 A2UI 编译、浏览器渲染和 Action 回传。
...
- Fixture 模式离线、确定性运行；
- 至少一个真实 UI Compiler 模型供应商 Smoke Test；
...
禁止 Web 直接调用 Business Agent、UI Compiler Service 或模型供应商。
...
### 4.5 UI Compiler Core

UI Compiler Core 是唯一可信 A2UI 生产者。

任何 Model Adapter 输出都必须经过 Schema、Policy、Catalog、Props、Action 和数据绑定校验后，才能进入确定性编译。

### 4.6 当前非目标

当前不实现 Interaction Gateway、多 Business Agent 路由、真实设备控制、生产数据库、权限计费、长期会话持久化和完整 A2UI 全规范。

## 5. 参考验证场景

开发环境使用“智能安防空地多智能体巡逻”作为参考领域，但平台核心保持业务无关。

- 查询设备状态：Business Agent 返回结构化设备数据，Compiler 生成状态展示。
- 生成巡逻计划：Business Agent 返回任务草稿，Compiler 生成计划和确认界面。
- 确认任务：用户点击 Action，Runtime 校验后恢复 LangGraph，再次编译并更新界面。
...
| TASK-004 | UI Compiler Model Adapter 多模型接入 | Fixture 与真实 Provider 的展示决策候选生成 |
| TASK-005 | Runtime Host 平台编排 | AgentContent 到 PresentationResult 的完整后端编排 |
| TASK-006 | Web Workbench 工程化 | 可发布的 Vue 开发联调工作台 |
| TASK-007 | Vue A2UI Renderer | 受控组件注册、数据绑定和 Surface 渲染 |
| TASK-008 | Action 回传闭环 | Action 校验、LangGraph Resume 和界面更新 |
| TASK-010 | 一键开发环境 | 四服务启动、环境检查和统一命令 |
| TASK-009 | 完整平台 E2E | Fixture 全链路和真实模型 Smoke |
| TASK-011 | 诊断与可观测性 | 跨服务关联、耗时、错误与安全诊断 |
...
TASK-005 + TASK-006 + TASK-007 → TASK-010
...
- Runtime Fixture 全链路不应被真实模型 Provider 接入阻塞。
