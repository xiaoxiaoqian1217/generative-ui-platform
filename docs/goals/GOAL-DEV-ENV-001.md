...
禁止 Web 直接调用 Business Agent、Presentation Pipeline 或模型供应商。
...
- 确认任务：用户点击 Action，Runtime 校验后恢复 LangGraph，再次调用 Pipeline 并更新界面。
...
| TASK-013 | Presentation Pipeline Package 提取 | 从旧 UI Compiler Service 提取可嵌入的展示应用能力 |
| TASK-004 | Presentation Model Adapter 多模型接入 | Fixture 与真实 Provider 的展示决策候选生成 |
...
| TASK-011 | 诊断与可观测性 | 跨模块关联、耗时、错误与安全诊断 |
...
- 不保留独立 UI Compiler HTTP Service 目标模式；
- Fixture 模式不需要模型密钥；
